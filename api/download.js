const { cors, extractId, getPlayer } = require("../lib");
  ...
  try {
    ...
    if (type === "mp3") {
      ...
      return res.status(200).json({
        videoId, title: data.videoDetails?.title, type: "audio",
    ...
    return res.status(200).json({
      videoId, title: data.videoDetails?.title, type: "video",
      quality: chosen.qualityLabel || `${chosen.height}p`,
      mime: chosen.mimeType, width: chosen.width, height: chosen.height,
      url: chosen.url,
      allOptions: mp4s.map(v => ({
        itag: v.itag,
        quality: v.qualityLabel || `${v.height}p`,
        mime: v.mimeType, width: v.width, height: v.height,
      })),
    });
  } catch (err) {
